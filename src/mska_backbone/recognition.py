...
from .tokenizers import GlossTokenizer_S2G
from .visual_head import VisualHead
from itertools import groupby

# PyTorch owns the CUDA device for model training and inference.  Keep the
# lightweight TensorFlow CTC decoder on CPU so the two frameworks do not fight
# over GPU memory during validation.
try:
    tf.config.set_visible_devices([], "GPU")
except RuntimeError:
    # TensorFlow raises if device visibility was already initialized.
    pass
